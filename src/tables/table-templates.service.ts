    ...
    private toEntity(dto: UpsertTableTemplateDto): Partial<TableTemplate> {
        return {
            ...
            columns: dto.columns.map((c) => ({
                ...
                // Hidden implies read-only: a field that isn't shown anywhere can't be
                // edited either. Enforced here so it also holds for columns set via
                // the raw JSON editor, not just the form's switches.
                readOnly: c.hidden ? true : (c.readOnly ?? false),
            ...
            allowBulkDelete: dto.allowBulkDelete ?? false,
        };
    }
}
